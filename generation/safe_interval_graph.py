...
def plot_safe_node_intervals(safe_node_intervals):
    node_map = dict()
        ...
        for movement in movements:
            for edge in movement:
                ...
                if node not in node_map:
                    node_map[node] = y
                    y += 1
                plt.plot([edge.depart_time, edge.depart_time], [node_map[edge.from_node.name], node_map[edge.to_node.name]], color='green', linestyle='--')
